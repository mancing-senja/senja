alter table public.senja_profiles
  add column if not exists minds jsonb not null default '{}'::jsonb
  check (jsonb_typeof(minds) = 'object' and octet_length(minds::text) <= 65536);
...
begin
  ...
  -- NPC relationship state is already sanitised by the room server. The DB
  -- still enforces shape/size so the public capability-token RPC cannot be
  -- abused to turn one profile into an unbounded JSON document.
  if jsonb_typeof(v_patch -> 'minds') = 'object'
    and octet_length((v_patch -> 'minds')::text) <= 65536 then
    for v_key, v_entry in select key, value from jsonb_each(v_patch -> 'minds') loop
      if v_key ~ '^[A-Za-z0-9:_-]{1,64}$'
        and jsonb_typeof(v_entry) = 'object'
        and octet_length(v_entry::text) <= 2048
        and ((v_row.minds ? v_key) or jsonb_object_length(v_row.minds) < 256) then
